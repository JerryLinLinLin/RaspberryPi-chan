...
class TextToSpeechService:
    def __init__(self, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        """
        self.device = device
        self.processor = AutoProcessor.from_pretrained(config.BARK_MODEL)
        self.model = BarkModel.from_pretrained(config.BARK_MODEL)
        self.model.to(self.device)

    def synthesize(self, text: str, voice_preset: str = config.BARK_VOICE_PRESET):
        """
